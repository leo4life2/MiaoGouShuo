...
- (void)setup {
    ...
    self.zanLabel.frame = CGRectMake(-100 ,- 100, 200, 40);
...
//    [self addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(pressOnece:)]];
    ...
    [self setImage:[UIImage imageNamed:@"likePink"] forState:UIControlStateNormal];
...
- (void)longPress:(UIGestureRecognizer *)ges
{
    ...
    if (ges.state == UIGestureRecognizerStateBegan) {
        ...
        [self changeText];
        [self showZanLabel];
        _timer = [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(longPressTimerAction) userInfo:nil repeats:YES];
    } else if (ges.state == UIGestureRecognizerStateEnded) {
        [self hiddenZanLabel];
        [_timer invalidate];
        _timer = nil;
        if (self.delegate != nil && [self.delegate respondsToSelector:@selector(likeButtonPressEnd:)]) {
            [self.delegate likeButtonPressEnd:countNum];
        }
    }
}
